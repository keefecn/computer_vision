...
""" step2:用Sobel算子计算x，y方向上的梯度，之后在x方向上减去y方向上的梯度，
...
""" step6：找出昆虫区域的轮廓。
cv2.findContours()函数第一个参数是要检索的图片，必须是为二值图，即黑白的（不是灰度图），
...
cv2.imwrite("../output/imgcut_Image6.jpg", image)
...
cv2.imshow("Image7", cropImg)
cv2.imwrite("../output/imgcut_Image7.jpg", cropImg)
